...
- (id)initWithFrame:(CGRect)frame {
    if ( (self = [super init]) ) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeImageView) name:MPMoviePlayerNowPlayingMovieDidChangeNotification object:nil];
...
-(void)changeSamllType
{
    ...
    self.view.center = samllPoint;
    [self.controls setFrame:CGRectMake(0, 0, smallFrame.size.width, smallFrame.size.height)];
}
...
//    [self play];
    ...
    UIImage *imgae = [VideoIcon getImage:contentURL];
...
- (void)play {
    ...
    [self setContentURL:[NSURL URLWithString:self.attachment.contents]];
